...
def quickconnect(event = None):
	if username.get().strip():
		...
		clientSocket.sendto(base64.b64encode(messageProtocol.encode('utf-8')), (quickserverIP, quickserverport))
		signIn.destroy()
...
def connect(event = None):
    if username.get().strip():
        ...
        clientSocket.sendto(
                base64.b64encode(
                    messageProtocol.encode('utf-8')
                ), 
                (str(serverIP.get()), int(port.get()))
        )
        signIn.destroy()
